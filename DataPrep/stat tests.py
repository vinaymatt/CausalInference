...
for sim in range(1, 2):
    ...
    file_path = "synthetic_data_with_infotreat4.xlsx"
    ...
    gp_model = TimeGPModel(time_train_tensor.unsqueeze(-1), Y_train_tensor, likelihood)
    ...
    generalization_error = mse.item()

    wasserstein_dist = compute_wasserstein(y_pred_treated, y_pred_untreated)
    generalization_error_bound = generalization_error + wasserstein_dist.item()

    print(f"ATE: {ate.item()}, Error: {error.item()}, Generalization Error: {generalization_error}, ")

    ate_and_error_list.append((ate.item(), error.item()))


with open(os.path.join(results_folder_name, 'ate_and_error_values.txt'), 'w') as f:
